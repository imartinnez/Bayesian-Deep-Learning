...
from src.data.target import add_future_vol_target, build_final_dataset, save_final_dataset, build_regime_labels
...
from src.data.splits import build_window_dataset_clf, save_window_dataset_clf, save_thresholds
from src.data.splits import save_parquet, save_scaler, build_window_dataset, save_window_dataset
...
if __name__ == "__main__":
    ...
    train_clf_windows_path = splits_dir / cfg["paths"]["train_clf_windows_filename"]
    val_clf_windows_path   = splits_dir / cfg["paths"]["val_clf_windows_filename"]
    test_clf_windows_path  = splits_dir / cfg["paths"]["test_clf_windows_filename"]
    clf_thresholds_path    = splits_dir / cfg["paths"]["clf_thresholds_filename"]

    low_pct  = cfg["classification"]["low_pct"]
    high_pct = cfg["classification"]["high_pct"]
    ...
    # --- Classification labels and sliding-window construction ---

    # Thresholds computed exclusively from training targets — no leakage.
    train_target_values = train_df_raw["target"].to_numpy()

    labeled_df, thresholds = build_regime_labels(
        scaled_df,
        train_target=train_target_values,
        target_col=target_col,
        low_pct=low_pct,
        high_pct=high_pct,
    )

    X_train_clf, y_train_clf, y_train_cont, dates_train_clf = build_window_dataset_clf(
        full_df=labeled_df,
        target_indices=train_indices,
        feature_cols=feature_cols,
        target_col=target_col,
    ...
    X_val_clf, y_val_clf, y_val_cont, dates_val_clf = build_window_dataset_clf(
        full_df=labeled_df,
        target_indices=val_indices,
        feature_cols=feature_cols,
        target_col=target_col,
    ...
    X_test_clf, y_test_clf, y_test_cont, dates_test_clf = build_window_dataset_clf(
        full_df=labeled_df,
        target_indices=test_indices,
        feature_cols=feature_cols,
        target_col=target_col,
    ...
    save_window_dataset_clf(X_train_clf, y_train_clf, y_train_cont, dates_train_clf, train_clf_windows_path)
    save_window_dataset_clf(X_val_clf,   y_val_clf,   y_val_cont,   dates_val_clf,   val_clf_windows_path)
    save_window_dataset_clf(X_test_clf,  y_test_clf,  y_test_cont,  dates_test_clf,  test_clf_windows_path)
    save_thresholds(thresholds, clf_thresholds_path)

    print(f"Train clf windows: {X_train_clf.shape} | labels: {dict(zip(*np.unique(y_train_clf, return_counts=True)))}")
    print(f"Val   clf windows: {X_val_clf.shape}   | labels: {dict(zip(*np.unique(y_val_clf, return_counts=True)))}")
    print(f"Test  clf windows: {X_test_clf.shape}  | labels: {dict(zip(*np.unique(y_test_clf, return_counts=True)))}")
