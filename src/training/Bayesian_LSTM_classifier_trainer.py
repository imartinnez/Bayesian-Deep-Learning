...
def predict(
    ...
    for X, y in loader:
        ...
        for _ in range(T):
            ...
            batch_p.append(probs.cpu().numpy())

        batch_p = np.stack(batch_p, axis=0)   # (T, batch, C)
        p_samples_all.append(batch_p)
        targets_list.append(y.cpu().numpy())

    p_samples = np.concatenate(p_samples_all, axis=1)   # (T, N, C)
    y_true    = np.concatenate(targets_list)              # (N,)

    summary = decompose_uncertainty_classification(p_samples)
...
def run_Bayesian_LSTM_classifier_training(cfg: dict, root_dir: Path) -> dict:
    ...
    checkpoint_path = models_dir  / cfg["paths"]["bayesian_clf_checkpoint"]
    loss_curve_path = figures_dir / cfg["paths"]["bayesian_clf_loss_curve_filename"]
    ...
    test_results = predict(model, test_loader, device, T=mc_samples)
